...
class EncodingServiceFactory:
    ...
    def get_instance(cls, encoding_scheme: EncodingScheme = EncodingScheme.V2) -> EncodingService:
        ...
        V12 and V13 use V12EncodingService (window projection + static vector output).
        All other schemes use the base EncodingService (RGBA uint8 output).
        ...
        if encoding_scheme not in cls._instances:
            ...
            elif encoding_scheme in (EncodingScheme.V12, EncodingScheme.V13):
                cls._instances[encoding_scheme] = V12EncodingService(encoding_scheme)
            else:
