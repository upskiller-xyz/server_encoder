...
class TestV7HeightDefaults:
    ...
    def test_provided_heights_are_not_overridden(self):
        """If the caller supplies heights, V7 must not override them."""
    ...
    def test_v7_encodes_without_height_params(self):
        """V7 must produce a valid image even when heights are not in the input."""
    ...
    def test_v7_image_matches_v4_with_same_heights(self):
        """V7 with explicit heights == V4 with the same heights (same channel mapping)."""
        params = dict(_BASE_PARAMS)
...
class TestV8HeightVector:
    ...
    def test_height_vector_overrides_individual_params(self):
        """height_vector takes precedence over any individually provided values."""
        params = dict(_BASE_PARAMS)
    ...
    def test_height_vector_missing_injects_defaults(self):
        """If height_vector is absent, V8 falls back to the same defaults as V7."""
        params = {k: v for k, v in _BASE_PARAMS.items()
                  if k not in ("height_roof_over_floor", "floor_height_above_terrain")}
        result = self._preprocess(params)
        assert result["height_roof_over_floor"] == _HEIGHT_DEFAULTS["height_roof_over_floor"]
    ...
    def test_v8_with_height_vector_matches_v7_with_same_heights(self):
        """V8 with height_vector=[H, F] must produce the same image as V7 with explicit H and F."""
        roof, floor = 18.0, 1.0
...
class TestV9ThreeChannel:
    """V9 must produce a 3-channel image; the room mask must come from the director."""
    ...
    def test_rgb_channels_match_v7(self):
        """The first 3 channels of V7 must equal the V9 output (same encoding, alpha just dropped)."""
        params = dict(_BASE_PARAMS)
        service_v7 = EncodingService(EncodingScheme.V7)
    ...
    def test_mask_is_not_none(self):
        """Director-derived room mask must be returned alongside the image."""
        service = EncodingService(EncodingScheme.V9)
        _, mask = service.encode_room_image_arrays(dict(_BASE_PARAMS), _MODEL)
        assert mask is not None

    def test_mask_is_binary(self):
        """Room mask must contain only 0 and non-zero values (binary room boundary)."""
        service = EncodingService(EncodingScheme.V9)
        _, mask = service.encode_room_image_arrays(dict(_BASE_PARAMS), _MODEL)
        unique = np.unique(mask)
        assert set(unique).issubset({0, 1} | set(unique[unique > 0])), (
            "Mask should be a binary room boundary"
        )

    def test_mask_does_not_cover_full_image(self):
        """The room occupies a fraction of the image — mask must not be all-ones."""
        service = EncodingService(EncodingScheme.V9)
        _, mask = service.encode_room_image_arrays(dict(_BASE_PARAMS), _MODEL)
        coverage = (mask > 0).mean()
        assert coverage < 1.0, f"Mask covers 100% of pixels — alpha-channel bug not fixed (coverage={coverage:.2f})"

    def test_v9_without_height_params_uses_defaults(self):
        """V9 inherits V7 preprocessing — heights are injected when absent."""
        service = EncodingService(EncodingScheme.V9)
        params = {k: v for k, v in _BASE_PARAMS.items()
                  if k not in ("height_roof_over_floor", "floor_height_above_terrain")}
        image, mask = service.encode_room_image_arrays(params, _MODEL)
...
class TestV10ThreeChannel:
    """V10 must produce a 3-channel image using heights from height_vector."""
    ...
    def test_rgb_channels_match_v9_with_same_heights(self):
        """V10 with height_vector=[H, F] must produce the same image as V9 with explicit H, F."""
        roof, floor = 18.0, 1.0
